...
def trans(amount, mode, account, reciever='Self'):
    if(mode == 1):
        if(int(check_balance(str(account))) >= 1000+int(amount)):
            ...
            if(a == 'y'):
                tid = transid(account)
                mycursor.execute("insert into trans values(%s,%s,%s,%s,%s)", (account, reciever, tid, date.today().strftime('%Y/%m/%d'), amount))
                mycursor.execute("insert into amount values(%s,%s,%s)", (tid, check_balance(str(account)) - amount, check_balance(str(account)) + amount))
                mycursor1.execute("Update user set balance = balance - %s where account = %s", (amount, account))
                mycursor1.execute("Update user set balance = balance + %s and transid = %s where account = %s", (amount, tid, reciever))
                mydb.commit()
    ...
    if(mode == 2):
        ...
        if(check_balance(str(account)) >= amount+1000):
            mycursor.execute("insert into trans values(%s,'self',%s,%s,%s)", (account, tid, date.today().strftime('%Y/%m/%d'), amount))
    ...
    if(mode == 3):
        ...
        mycursor.execute("insert into amount values(%s,%s,%s)", (tid, "Null", check_balance(str(account))+int(amount)))
        mycursor.execute("update user set balance = balance + %s where account = %s", (amount, account))
    ...
    if(mode == 4):
        tid = transid(account)
        mycursor.execute("insert into trans values(%s,'self',%s,%s,%s)", (account, tid, date.today().strftime('%Y/%m/%d'), check_balance(str(account))))
            ...
            if(a in range(1, 4)):
                if(a == 1):
                    ...
                    if(istransid(account, transid)):
                        ...
                        mycursor1.execute("select transid, sender, beneficiary, date, Beneficiary_amount from trans natural join amount where trans.transid = %s and beneficiary = %s", (transid,account))
                ...
                if(a == 2):  ## Needed to be recoded
                    ...
                    if(b[0] is True):
                        ...
                        c = date_input()
                        if(c[0]):
                            mycursor.execute("select transid, sender, beneficiary, date, sender_amount from trans natural join amount where sender = %s and Date between %s and %s", (str(account),b[1],c[1]))
                            e = mycursor.fetchall()
                            mycursor1.execute("select transid, sender, beneficiary, date, beneficiary_amount from trans natural join amount where Date between %s and %s and beneficiary = %s", (b[1],c[1],account))
...
def close_account( account):
        ...
        if(check_balance(str(account)) == '0'):
            k = input("Are you Sure to delete Account, Press Y to continue, Else press any key to exit").lower()
            if(k!='y'):
                return "Operation Cancelled"
            mycursor.execute("delete from user where account = %s", (account,))
            mydb.commit()
            return "Account deleted Successfully"
        k = check_balance(str(account))
        trans(k, 3, account)
        k = input(f"Are you Sure to delete Account with account number {account}, Press Y to continue, Else press any key to exit").lower()
        if(k!='y'):
            return "Operation Cancelled"
        mycursor.execute("delete from user where account = %s", (account,))
